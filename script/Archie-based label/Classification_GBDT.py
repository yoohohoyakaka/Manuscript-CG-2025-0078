...
cv_scores = cross_val_score(gbdt_classifier, X_scaled, y, cv=cv, scoring="accuracy")

# Evaluate
y_pred_non_group = gbdt_classifier.predict(X_test)
y_proba_non_group = gbdt_classifier.predict_proba(X_test)[:, 1]
...
# Create groups (3-meter interval)
...
cv_scores_group = cross_val_score(
    gbdt_classifier, X_train_g, y_train_g,
...
# Train and evaluate
gbdt_classifier.fit(X_train_g, y_train_g)
y_pred_group = gbdt_classifier.predict(X_test_g)
y_proba_group = gbdt_classifier.predict_proba(X_test_g)[:, 1]
